    ...
    async def get_model(self, model_type: ModelType) -> Any:
        ...
        async with self._locks[model_type]:
            ...
            if not model_config.get("enabled", False):
                raise ValueError(
                    f"Model {model_type.value} is not enabled in configuration"
                )
        ...
        try:
            if model_type == ModelType.BINARY_CLASSIFIER:
                ...
                adapter = SegmentationAdapter(
                    checkpoint_path=model_config.get("checkpoint_path"),
                    model_type=model_config.get("model_type", "pspnet"),
                    device=self.device,
                )
                ...
                adapter = FeatureExtractorAdapter(
                    model_name=model_config.get("model_name", "google/medsiglip-448"),
    ...
    async def unload_model(self, model_type: ModelType) -> None:
        ...
        async with self._locks[model_type]:
            if model_type in self.models:
                ...
                logger.warning(
                    f"Model {model_type.value} not loaded, nothing to unload"
                )
